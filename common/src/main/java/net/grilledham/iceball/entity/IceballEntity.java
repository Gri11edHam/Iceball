...
import net.minecraft.world.entity.projectile./*? if >= 1.21.11 >>+ '.'*/throwableitemprojectile.ThrowableItemProjectile;
...
public class IceballEntity extends ThrowableItemProjectile {
	...
	public IceballEntity(Level world, LivingEntity owner, ItemStack stack, int damage, BiFunction<IceballEntity, HitResult, Boolean> onCollide, Consumer<IceballEntity> onTick) {
		...
		//?} else
		//super(EntityType.SNOWBALL, owner, world/*? if >= 1.21.11 >> ')'*/, stack);
		this.damage = damage;
	...
	public IceballEntity(Level world, Position pos, ItemStack stack, int damage, BiFunction<IceballEntity, HitResult, Boolean> onCollide, Consumer<IceballEntity> onTick) {
		...
		//?} else
		//super(EntityType.SNOWBALL, pos.x(), pos.y(), pos.z(), world/*? if >= 1.21.11 >> ')'*/, stack);
		this.damage = damage;
		this.onCollide = onCollide;
		this.onTick = onTick;
	}
	...
	protected void onHitEntity(EntityHitResult entityHitResult) {
		...
		if (this.level() instanceof ServerLevel serverWorld) {
			//? if >= 1.21.11 {
			entity.hurtServer(serverWorld, serverWorld.damageSources().thrown(this, this.getOwner()), (float)i);
			//?} else
			//entity.hurt(serverWorld.damageSources().thrown(this, this.getOwner()), (float)i);
		}
	}
