...
import net.minecraft.world.entity.vehicle./*? if >= 1.21.11 >>+ '.'*/boat.Boat;
import net.minecraft.world.item.Item;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.item.component.DyedItemColor;
import net.minecraft.world.level.Level;
import net.minecraft.world.level./*? if >= 1.21.11 >>+ '.'*/gamerules.GameRules;
//? if >= 1.21.11 {
...
public class BigBouncyBallEntity extends VehicleEntity implements Leashable, PlayerRideableJumping {
	...
	public BigBouncyBallEntity(Level world, double x, double y, double z) {
		this(IceballEntities.BIG_BOUNCY_BALL_ENTITY, world);
		setPos(x, y, z);
		xOld = x;
		yOld = y;
		zOld = z;
	...
	*///?}
		...
		if(getBallColor() != 0xFF88DD88) {
			itemStack.set(DataComponents.DYED_COLOR, new DyedItemColor(getBallColor() & 0xFFFFFF/*? if < 1.21.11 >> ')'*//*, true*/));
		}
		this.spawnAtLocation(/*? if >= 1.21.11 >>+ ' '*/world, itemStack);
	}
	
	@Override
	protected Item getDropItem() {
		return IceballItems.BIG_BOUNCY_BALL_ITEM;
	}
	...
	@Override
	public boolean canBeCollidedWith(/*? if >= 1.21.11 >> ')'*/@Nullable Entity entity) {
		return true;
	}
	...
	//~ if >= 1.21.11 'getAccumulatedTime()' -> 'getTimeInMillis(tickCount)' {
	@Override
...
//			this.updateVelocity();
	...
	@Override
	public InteractionResult interact(Player player, InteractionHand hand/*? if >= 26.1 >> ')'*/, Vec3 location) {
		InteractionResult superInteraction = super.interact(player, hand/*? if >= 26.1 >> ')'*/, location);
	...
	@Override
	public boolean causeFallDamage(/*$ if >= 1.21.11 'double' else 'float' >>*/double fallDistance, float damageMultiplier, DamageSource damageSource) {
		return false;
	}
	...
	*///?}
	...
	@Override
	protected void addAdditionalSaveData(/*$ if >= 1.21.11 'ValueOutput' else 'CompoundTag' >>*/ValueOutput nbt) {
