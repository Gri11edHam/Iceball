...
tasks {
    ...
    processResources {
        ...
        if(stonecutterBuild.eval(stonecutterBuild.current.version, ">= 1.21.11")) {
            exclude("data/minecraft/tags/item/dyeable.json")
        } else {
            exclude("data/iceball/recipe/*_dyed.json")
        }
    }
...
val generateIdeaRunConfig = tasks.register("generateIdeaRunConfig") {
    doLast {
        ...
        val loaderName = requireNotNull(prop("loader-name"))
        ...
        listOf("Client", "Server").forEach { side ->
            ...
            if (current != text) {
                ...
                if(side == "Server") {
					...
					eula=true
					""".trimIndent()
                    val eulaFile = rootProject.file("run/server/eula.txt")
...
val buildAndCollect = tasks.register<Copy>("buildAndCollect") {
    ...
    val version = stonecutterBuild.current.version
    group = "build"
    description = "Copies all artifacts into build directory"
    from(rootProject.file("$loader/versions/$version/build/libs"))
    into(rootProject.layout.buildDirectory.file("libs/$version/$loader"))
    dependsOn("build")
}
